  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createStory.fulfilled, (state, action) => {
        ...
        // Add new story to the appropriate user group or create new group
        const newStory = action.payload;
        const existingUserGroup = state.userStories.find(
          group => group._id._id === newStory.userId._id
        );
      ...
      .addCase(viewStory.fulfilled, (state, action) => {
        const viewedStory = action.payload;
        // Update the story in the state
        state.userStories.forEach(userGroup => {
          const storyIndex = userGroup.stories.findIndex(s => s._id === viewedStory._id);
          if (storyIndex !== -1) {
            userGroup.stories[storyIndex] = viewedStory;
          }
        });
      })
